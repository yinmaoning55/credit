...
import com.neuedu.credit.util.HBaseUtil;
...
public class IndexController {
    ...
    public String helloWorld(@PathVariable("username") String username) {
        return "Hello  " + username + " , the world welcome you !" + new Date();
    }
    @RequestMapping("/index")
    public String index(Model model){
        String xname = "lixiaoming";
        model.addAttribute("name",xname);
        return "/other/index";
    }
    private static Process process;//执行子进程
    private static String Contexttxt;

    @RequestMapping(value = "/compiler", method = RequestMethod.POST)
    @ResponseBody
    public String compiler(HttpServletRequest req, HttpServletResponse response) throws ServletException, IOException {
...
//执行时间也是无法知道，因为dos执行java命令，程序无法知道它到底执行到那里了，两个进程，互不了解
                ...
                bw.close();
//获取控制台输出的结果
                Thread runtimeInput = new Thread(new IndexController.RuntimeInput());
                runtimeInput.start();
...
//如果你有办法知道，可以留言,现在得到程序运行前后内存使用率，不过输出的是0，因为程序退出，不占内存了
    ...
    @ResponseBody
    @RequestMapping("/addbase/{rows}")
    public String addHbase(@PathVariable int rows){
        for (int i = 0; i < rows; i++) {
            String rowkey = String.valueOf(10000+i);
            String[] colunms = new String[]{
                    "column1",
                    "column2"
            };
            String[] values = new String[]{
                    String.valueOf (Math.random()*i),
                    String.valueOf (Math.random()*i)
            };
            HBaseUtil.putRow("testTableCreater",rowkey,"family1",colunms,values);
        }
        return "tj cg ";
    }
    @ResponseBody
    @RequestMapping("/getbase")
    public String getHBaseRow() {
        String[] columns = new String[]{
                "column0",
                "column1"
        };
        Result result = HBaseUtil.getRow("testTableCreater","family1","990000",columns);
        String col1 = "column1 :"+ Bytes.toString( result.getValue(Bytes.toBytes("family1"),Bytes.toBytes("column1")));
        String col2 = "column2 :"+ Bytes.toString( result.getValue(Bytes.toBytes("family1"),Bytes.toBytes("column2")));
